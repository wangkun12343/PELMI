            ...
            for correct_name, error_correction in self.error_corrections.items():
                for file_name, file_path in self.file_paths.items():
                    ...
                    for iteration in range(self.iterations):
                        ...
                        t1 = len(dna_sequences[0])
                        t2 = self.nucleotide_insertion

                        # insertion errors
                        for insertion_iteration in range(int(len(dna_sequences) * self.nucleotide_insertion * t1) ):
                        ...
                        # mutation errors
                        for mutation_iteration in range(int(len(dna_sequences) * self.nucleotide_mutation * t1) ):
                            chosen_index = random.choice(total_indices)
                        ...
                        # deletion errors
                        for deletion_iteration in range(int(len(dna_sequences) * self.nucleotide_deletion * t1)):
                            chosen_index = random.choice(total_indices)
